...
// IRQ Control
BYTE nIRQ = 5;                                     // IRQ Number
HIRQ hIRQ = 0;                                     // IRQ Handle
VID IRQDesc;                                       // VPCID_IRQ_Descriptor
DWORD sIRQ = 0;                                    // IRQ Status

//UART Control
DWORD addrUART = 0x368;                            // UART base address
DWORD addrMx = 0x360;                              // channel multiplexsor
WORD koefSpeed[SPEEDS] =
       {2880,1440,720,480,240,120,60,30,15};       // speed koefficients
...
WORD trapFlags[CHANNELS] = {0,0,0,0,0,0,0,0};      // channel's trap flags
WORD messageFlags[CHANNELS] = {0,0,0,0,0,0,0,0};   // channel's message flags
BYTE existInt[CHANNELS] = {0,0,0,0,0,0,0,0};       // UART is existing
HTIMEOUT hTimeState[CHANNELS] = {0,0,0,0,0,0,0,0}; // state timeout handlers
HTIMEOUT hTimePulse[CHANNELS] = {0,0,0,0,0,0,0,0}; // pulse timeout handlers
...
VOID  CVXD_Hw_Int(void)
{
    ...
    while (TRUE)
    {
      ...
      req = CVXD_What_Are_IRQs();
      if (req == 0) break;

      // all interrups process
      for (channel = 0; channel <= CHANNELS; channel++)
      {
        if ((req & (1 << channel)) == 0) continue;
